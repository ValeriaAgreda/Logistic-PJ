...
const Documents = () => {
  ...
  const validar = (documento) => {
    ...
    if (documento.archivo_nombre && !esArchivoPermitido(documento.archivo_nombre)) {
      e.ruta_documento = MENSAJE_ARCHIVO_INVALIDO;
    }
  ...
  const seleccionarArchivo = async (archivo, state, setState) => {
    ...
    if (!esArchivoPermitido(archivo.name)) {
      ...
      setErrores((prev) => ({ ...prev, ruta_documento: MENSAJE_ARCHIVO_INVALIDO }));
      return;
    }
    ...
    try {
      ...
      setErrores((prev) => ({ ...prev, ruta_documento: undefined }));
    } catch (error) {
  ...
  const documentosFiltrados = useMemo(() => {
    ...
    return documentos.filter((documento) => !q || [documento.tipo_documento, documento.codigo_operacion, documento.numero_documento, obtenerNombreDocumento(documento.ruta_documento), documento.descripcion].some((v) => String(v || "").toLowerCase().includes(q)));
  }, [documentos, search]);
    ...
    return (
      <div className="mb-3">
        ...
        <div className="form-control bg-light">
          {documento?.archivo_base64 || !documento?.ruta_documento ? (
            <span>{nombre}</span>
          ) : (
