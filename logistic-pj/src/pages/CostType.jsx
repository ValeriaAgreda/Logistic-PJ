...
const CostType = () => {
  ...
  const cargarTiposCosto = async () => {
    try {
      const res = await fetch("http://localhost:3001/api/tipo-costo", {
        credentials: "include",
      });
      ...
      if (!res.ok) {
        console.error(data?.error || "Error al obtener tipos de costo");
        return;
      }

      setTiposCosto(Array.isArray(data) ? data : []);
    } catch (error) {
      console.error("Error al obtener tipos de costo:", error);
    }
  };
  ...
  const abrirNuevo = () => {
    ...
    setErrores({});
    new bootstrap.Modal(document.getElementById("addTipoCostoModal")).show();
  };
  ...
  const abrirEditar = (tipo) => {
    ...
    setErrores({});
    new bootstrap.Modal(document.getElementById("editTipoCostoModal")).show();
  };

  const abrirEliminar = (tipo) => {
    if (!tipo) return;
    setTipoAEliminar(tipo);
    new bootstrap.Modal(document.getElementById("deleteTipoCostoModal")).show();
  };
  ...
  const guardarNuevo = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        alert(data?.error || "Error al crear tipo de costo");
        return;
      }

      await cargarTiposCosto();
      bootstrap.Modal.getInstance(
        document.getElementById("addTipoCostoModal")
      )?.hide();
      setNuevoTipo(tipoInicial);
      setErrores({});
    } catch (error) {
      console.error("Error al crear tipo de costo:", error);
  ...
  const guardarEdicion = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:3001/api/tipo-costo/${tipoSeleccionado.id_tipo_costo}`,
        {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      if (!res.ok) {
        alert(data?.error || "Error al actualizar tipo de costo");
        return;
      }

      await cargarTiposCosto();
      bootstrap.Modal.getInstance(
        document.getElementById("editTipoCostoModal")
      )?.hide();
  ...
  const eliminarTipoCosto = async () => {
    if (!tipoAEliminar) return;

    try {
      const res = await fetch(
        `http://localhost:3001/api/tipo-costo/${tipoAEliminar.id_tipo_costo}`,
        {
          method: "DELETE",
          credentials: "include",
        }
      ...
      if (!res.ok) {
        alert(data?.error || "Error al eliminar tipo de costo");
        return;
      }

      await cargarTiposCosto();
      bootstrap.Modal.getInstance(
        document.getElementById("deleteTipoCostoModal")
      )?.hide();
      setTipoAEliminar(null);
      setSelectedId(null);
    } catch (error) {
      console.error("Error al eliminar tipo de costo:", error);
      alert("Error en el servidor");
    }
  };

  const tipoSeleccionadoTabla = useMemo(
    () => tiposCosto.find((t) => t.id_tipo_costo === selectedId) || null,
    [tiposCosto, selectedId]
  );
    ...
    <>
      ...
      <div className="page-container flex-grow-1 p-4">
        <div className="d-flex justify-content-between align-items-center mb-3">
          ...
          {tipoSeleccionadoTabla ? (
            ...
            <small className="text-muted">
              Selecciona un tipo de costo para Editar/Eliminar
            </small>
          )}
        </div>
          ...
          <table className="table table-hover table-bordered align-middle m-0">
            ...
            <tbody>
              {tiposFiltrados.map((t, idx) => {
                const isSelected = t.id_tipo_costo === selectedId;

                return (
                  <tr
                    key={t.id_tipo_costo}
                    className={isSelected ? "row-selected" : ""}
                    onClick={() => setSelectedId(t.id_tipo_costo)}
        ...
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content shadow rounded-3">
            <div className="modal-header">
              <h5 className="modal-title">Agregar tipo de costo</h5>
      ...
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content shadow rounded-3">
            <div className="modal-header">
              <h5 className="modal-title">Editar tipo de costo</h5>
              <button className="btn-close" data-bs-dismiss="modal" />
            </div>
            <div className="modal-body">
              {tipoSeleccionado && (
      ...
      >
        <div className="modal-dialog modal-dialog-centered">
          <div className="modal-content shadow rounded-3">
            <div className="modal-header">
              <h5 className="modal-title">Eliminar tipo de costo</h5>
              <button className="btn-close" data-bs-dismiss="modal" />
            </div>
            <div className="modal-body">
              {tipoAEliminar && (
            ...
            <div className="modal-footer">
              ...
              <button className="btn btn-danger" onClick={eliminarTipoCosto}>
                Eliminar
              </button>
            </div>
          </div>
